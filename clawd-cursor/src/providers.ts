...
export const PROVIDERS: Record<string, ProviderProfile> = {
  ...
  gemini: {
    ...
    openaiCompat: false,
    computerUse: true, // Gemini vision-based computer use loop
  },
  vertexai: {
    name: "Google Vertex AI",
    baseUrl: "https://vertexai.googleapis.com",
    authHeader: () => ({}), // Uses ADC (gcloud auth)
    textModel: "gemini-3-flash-preview",
    visionModel: "gemini-3-flash-preview",
    openaiCompat: false,
    computerUse: true,
...
export function detectProvider(
  ...
  if (apiKey.startsWith("AIza")) return "gemini"; // Google API keys start with AIza
...
export async function scanProviders(): Promise<ProviderScanResult[]> {
  ...
  if (resolvedApi.source === "openclaw") {
    ...
    try {
      ...
      /* OpenClaw config read failed, continue with existing logic */
    }

    if (Object.keys(openclawProviderKeys).length > 0) {
      console.log(
        `   🔗 OpenClaw providers detected: ${Object.keys(openclawProviderKeys).join(", ")}`,
      );
    }
  }
  ...
  // ── Check Vertex AI (uses ADC from gcloud, no API key needed) ──
  if (process.env.USE_VERTEXAI === "true" && process.env.VERTEXAI_PROJECT) {
    const vertexResult = results.find((r) => r.key === "vertexai");
    if (!vertexResult) {
      results.push({
        key: "vertexai",
        name: PROVIDERS["vertexai"].name,
        available: true,
        detail: `project=${process.env.VERTEXAI_PROJECT}, location=${process.env.VERTEXAI_LOCATION || "global"} (ADC)`,
        apiKey: "", // Uses gcloud ADC
      });
    }
  }
  ...
  if (resolvedApi.source === "openclaw") {
    try {
        ...
        for (const configPath of configPaths) {
          try {
            ...
            for (const [provName, provConfig] of Object.entries(providers)) {
              ...
              const knownProvider = Object.values(PROVIDERS).some(
                (p) =>
                  p.baseUrl === baseUrl ||
                  providerNameLower.includes(
              ...
              for (const authPath of authPaths) {
                try {
                  ...
                  for (const [profileKey, profileValue] of Object.entries(
                    ...
                    const profileProviderName = profileKey
                      .split(":")[0]
                      .toLowerCase();
              ...
              const textModels = Object.keys(models).filter(
                (m) =>
                  !m.toLowerCase().includes("vision") &&
                  !m.toLowerCase().includes("dall-e") &&
                  !m.toLowerCase().includes("tts"),
              );
              const visionModels = Object.keys(models).filter(
                (m) =>
                  m.toLowerCase().includes("vision") ||
              ...
              const dynamicProviderKey = providerNameLower.replace(
                /[^a-z0-9]/g,
                "",
              );
      ...
      /* OpenClaw dynamic provider creation failed, continue */
    }
  }
  ...
  for (const result of results) {
    if (openclawProviderKeys[result.key]?.baseUrl && result.available) {
      ...
      (result as any).openclawBaseUrl =
        openclawProviderKeys[result.key].baseUrl;
    }
  }
...
const TEXT_MODEL_PREFERENCE: string[] = [
  "ollama",
  "groq",
  "gemini",
  "vertexai",
...
export function buildMixedPipeline(
  ...
  const primaryKey =
    bestVision?.providerKey || bestText?.providerKey || "ollama";
