...
export class SmartInteractionLayer {
  ...
  async tryHandle(
    ...
    if (!this.isAvailable()) {
      return {
        ...
        steps: [],
        llmCalls: 0,
        description: "SmartInteraction disabled",
      };
    }
    ...
    try {
      ...
      if (this.isVisualLoopTask(task)) {
        console.log(
          `   ⏭️  Smart Interaction: visual loop task detected — handing off to Computer Use`,
        );
        return {
          handled: false,
          success: false,
          steps: [],
          llmCalls: 0,
          description: "Visual loop task — Computer Use required",
        };
      }
      ...
      if (this.isDescribeTask(task)) {
        ...
        if (!result.handled || !result.success) {
          console.log(
            `   🔄 Smart Interaction: CDP path failed — trying UIDriver (accessibility tree)`,
          );
      ...
      if (result.handled && result.success) {
        ...
        console.log(
          `   ✅ Smart Interaction handled in ${Date.now() - startTime}ms (${result.llmCalls} LLM call)`,
        );
      }
      ...
      console.log(
        `   ⚠️ Smart Interaction error (${this.consecutiveFailures}/${this.MAX_FAILURES}): ${err}`,
      );

      if (this.consecutiveFailures >= this.MAX_FAILURES) {
        this.disabled = true;
        console.log(
          `   🔴 Smart Interaction circuit breaker tripped — disabled for this session`,
        );
      }
      ...
      return {
        ...
        steps: [
          {
            action: "error",
            description: `Smart Interaction error: ${err}`,
            success: false,
            timestamp: Date.now(),
          },
        ],
  ...
  private async handleBrowserTask(
    ...
    const connected =
      (await this.cdpDriver.isConnected()) || (await this.cdpDriver.connect());
    if (!connected) {
      console.log(
        `   ⚠️ Smart Interaction: CDPDriver can't connect to CDP port — falling through`,
      );
      return {
        handled: false,
        success: false,
    ...
    if (!pageContext || pageContext.includes("unavailable")) {
      return {
        ...
        steps: [],
        llmCalls: 0,
        description: "Page context unavailable",
      };
    }
    ...
    if (!plan || !plan.canHandle) {
      console.log(
        `   🤷 Smart Interaction: LLM says can't handle — ${plan?.reasoning || "no plan"}`,
      );
      return {
        handled: false,
        success: false,
        steps: [
          {
            action: "plan",
            description: `Can't handle: ${plan?.reasoning || "unknown"}`,
            success: false,
            timestamp: Date.now(),
          },
        ],
    ...
    if (criticalFailure) {
      return {
        ...
        steps,
        llmCalls: 1,
        description: "Critical step failed",
      };
    }
    ...
    try {
      switch (step.action) {
        ...
        case "type": {
          ...
          return {
            action: "type",
            description: `Type "${step.text}" into "${step.target}"`,
            success: result.success,
            error: result.error,
            timestamp: ts,
          };
        ...
        case "select": {
          ...
          return {
            action: "select",
            description: `Select "${step.text}" in "${step.target}"`,
            success: result.success,
            error: result.error,
            timestamp: ts,
          };
        ...
        case "wait": {
          ...
          return {
            action: "wait",
            description: `Wait ${step.waitMs || 1000}ms`,
            success: true,
            timestamp: ts,
          };
        }
        ...
        case "navigate": {
          ...
          if (page && step.target) {
            ...
            if (
              ...
              return {
                action: "navigate",
                description: `Already at ${step.target} — skipped`,
                success: true,
                timestamp: ts,
              };
            }
          ...
          return {
            action: "navigate",
            description: `Navigate to ${step.target}`,
            success: true,
            timestamp: ts,
          };
        }
        ...
        case "skip":
          return {
            action: "skip",
            description: step.target || "Skipped",
            success: true,
            timestamp: ts,
          };
  ...
  private async handleNativeTask(
    task: string,
  ): Promise<SmartInteractionResult> {
    const steps: StepResult[] = [];
    ...
    if (openAppMatch) {
      ...
      const activeWindowProcess = (
        activeWindow?.processName || ""
      ).toLowerCase();
      if (
        !activeWindowTitle.includes(targetApp) &&
        !activeWindowProcess.includes(targetApp)
      ) {
        console.log(
          `   ⏭️ Smart Interaction: "${targetApp}" not in active window — skipping to Computer Use`,
        );
        return {
          handled: false,
          success: false,
          steps: [],
          llmCalls: 0,
          description: `Target app "${targetApp}" not active`,
        };
      }
    }

    const a11yContext = await this.a11y
      .getScreenContext(activeWindow?.processId)
      .catch(() => "");
    ...
    if (!a11yContext || a11yContext.includes("unavailable")) {
      return {
        ...
        steps: [],
        llmCalls: 0,
        description: "A11y context unavailable",
      };
    }
    ...
    try {
      switch (step.action) {
        ...
        case "expand": {
          ...
          return {
            action: "expand",
            description: `Expand "${step.target}"`,
            success: result.success,
            error: result.error,
            timestamp: ts,
          };
        ...
        case "select": {
          ...
          return {
            action: "select",
            description: `Select "${step.target}"`,
            success: result.success,
            error: result.error,
            timestamp: ts,
          };
        ...
        case "wait": {
          if (step.target) {
            const el = await ui.waitForElement(
              step.target,
              step.waitMs || 5000,
            );
            return {
        ...
        default:
          return {
            action: step.action,
            description: `Unknown native action: ${step.action}`,
            success: false,
            timestamp: ts,
          };
      }
  ...
  private isVisualLoopTask(task: string): boolean {
    ...
    const hasLoop =
      /\b(loop|repeat|keep doing|every time|until (done|complete|nothing left|finished))\b/.test(
        t,
      );
    const hasScreenshot =
      /\b(screenshot|take a screenshot|screen capture)\b/.test(t);
    const hasWaitAndRespond =
      /\b(wait for.*(respond|response|reply)|monitor progress)\b/.test(t);
    // Drawing tasks require visual feedback loops (draw -> look -> adjust -> repeat)
    const isDrawingTask = /\bdraw\b/i.test(task);
    return (
      (hasLoop && hasScreenshot) ||
      (hasLoop && hasWaitAndRespond) ||
      isDrawingTask
    );
  }
  ...
  private isDescribeTask(task: string): boolean {
    const t = task.trim();
    return (
      /^(describe|what(?:'s| is)|tell me|show me|explain)\s+(what'?s?\s+)?(on|the|in|about)?\s*(screen|page|window|app|visible|open|current)/i.test(
        t,
      ) ||
      /^what(?:'s| is)\s+(on\s+)?(my\s+)?(screen|page|window|display)/i.test(
        t,
      ) ||
   ...
   */
  private async handleDescribeTask(
    task: string,
  ): Promise<SmartInteractionResult> {
    console.log(
      `   🔍 Smart Interaction: describe task detected — using a11y context directly`,
    );
    ...
    if (!a11yContext || a11yContext.includes("unavailable")) {
      console.log(
        `   ⚠️ Smart Interaction: a11y context unavailable for describe task — falling through`,
      );
      return {
        handled: false,
        success: false,
    ...
    const description = await this.callTextModel(
      userMessage,
    ...
    if (!description) {
      return {
        ...
        steps: [],
        llmCalls: 1,
        description: "Description LLM call failed",
      };
    }
  ...
  private async callTextModel(
    ...
    const model =
      providerProfile.textModel || this.config.ai.model || "gpt-4o-mini";
    const baseUrl =
  ...
  private async callLLM(
    ...
    const isGemini =
      ...
      (baseUrl.includes("vertexai.googleapis.com") ||
        baseUrl.includes("generativelanguage.googleapis.com") ||
        provider.name.toLowerCase().includes("gemini") ||
        provider.name.toLowerCase().includes("vertex") ||
        process.env.USE_VERTEXAI === "true");

    if (isGemini) {
      const { geminiTextCall, buildGeminiConfig } =
        await import("./gemini-client");
      const gConfig = buildGeminiConfig({ model });
      return geminiTextCall(gConfig, systemPrompt, userMessage, 500);
    }
